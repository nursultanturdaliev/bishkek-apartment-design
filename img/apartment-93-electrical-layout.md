...
**Версия документа:** 1.2 · **Дата:** 2026-04-19 · **Кратко:** добавлены **§5.10.A / §5.10.B** — парентское крыло по эталонному изображению заказчика (en-suite 3,1 м² + спальня 15,1 м² L-shape + лоджия 4,4 м²), новый SVG-план с дименшнами, ведомость электрики, **TBD-E10** на верификацию обмером; пометки в **§5.7 / §5.8 / §5.9** о новой функции мокрых зон; **§4.1** оверлей плана и design-book — под обновление после обмера. Прежняя v1.1 — визуальные схемы (§3.1 / §4.1 / §5.6.A / §6.1 / §7.1 / §8.1 / §10.1).
...
5. [По помещениям — детальные ведомости](#section-5) — [§5.6.A IP-zones reference SVG](#section-5-wet-zones)
   ...
   - [5.10.A Парентское крыло — план по эталону заказчика (en-suite + спальня + лоджия)](#section-5-10-a)
   - [5.10.B Ведомость электрики парентского крыла](#section-5-10-b)
   - [5.10 Спальня ~15,1 м² — типовой шаблон](#section-5-10)
...
> ⚠️ **Поправка от 2026-04-19 (TBD-E10):** парентское крыло (запад) **переинтерпретировано** по эталонному изображению заказчика — добавлен **парентский en-suite (3,1 м²)** на севере крыла, центральный мокрый блок **4,2 м²** теперь **детский / основной** санузел. Полная **перерисовка** этого оверлея — **после обмера**; до тех пор **источник истины для парентского крыла** — **[§5.10.A](#section-5-10-a)** (отдельный SVG-план с дименшнами). На этом оверлее ниже парентское крыло отрисовано по **прежней** интерпретации (общая ванная) — учитывать как **устаревшее** для запада.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="-2200 -2300 11400 19400" role="img" aria-label="Electrical zone overlay on apartment 93 floor plan: lighting groups, AC indoor units, boiler position, LAN backbone star from pantry">
  ...
  <!-- TBD-E10 callout: parents wing re-scoped per §5.10.A -->
  <rect x="100" y="13000" width="2700" height="650" fill="#fff5f5" stroke="#c53030" stroke-width="40" stroke-dasharray="180 100"/>
  <text x="1450" y="13280" font="700 200px system-ui, sans-serif" fill="#c53030" text-anchor="middle">TBD-E10 · re-scoped</text>
  <text x="1450" y="13510" font="160px system-ui, sans-serif" fill="#742a2a" text-anchor="middle">+ en-suite 3.1 m² (NW) — see §5.10.A</text>
...
> **Функция по новой интерпретации (§5.10.A):** **гостевой WC** у входа (без изменений). Используется приходящими и членами семьи в зоне холла.
...
> **Функция по новой интерпретации (§5.10.A):** этот **3,1 м²** — теперь **парентский en-suite** в крыле родителей (запад). Содержательная ведомость для en-suite — в **§5.10.B** (часть 1). Таблица ниже остаётся как **типовой шаблон** для совмещённого узла WC + раковина + душ — пользоваться при сверке решений по сантехнике и IP-зонам.
...
> **Функция по новой интерпретации (§5.10.A):** **детский / основной санузел** — главный «семейный» узел между холлом и спальней детей (центр-юг). Стирка / сушка остаются здесь (см. ниже). Гости и посетители пользуются **§5.7 гостевым WC** у входа.
...
<a id="section-5-10-a"></a>

### 5.10.A Парентское крыло — план по эталонному изображению заказчика (2026-04-19)

> **Источник:** скриншот заказчика от 2026-04-19 16:50 (см. `assets/Screenshot_2026-04-19_at_16.50.01-…png`). Заказчик обозначил это как **парентское крыло** с **отдельным санузлом (en-suite)** на севере, **спальней** в середине и **лоджией** на юге.
>
> **Статус:** **TBD-E10** — план **выведен** из изображения и площадей госреестра; **числа в мм — гипотеза**, любая отметка **сверяется на объекте лентой** до штробления и заказа корпусной мебели.

**Допущения, ставшие основой геометрии (фиксируются в §3.3.1 требований при подтверждении):**

| Допущение | Значение | Источник |
|-----------|----------|----------|
| Внутренняя ширина крыла (W ↔ E) | **2700 мм** | Совпадает с проектным envelope design-book (§4.6 design-book) |
| Внутренняя длина крыла без лоджии (N ↔ S) | **6750 мм** | Подобрано так, чтобы en-suite (3,1 м²) + спальня (15,1 м²) совпали с госреестром |
| En-suite (3,1 м²) | **1700 × 1850 мм** в NW-углу | Площадь госреестра = 3,15 м² |
| Спальня (15,1 м²) | L-образная: коридор 1000 × 1850 (E от en-suite) + основной объём 2700 × 4900 | Сумма = 15,08 м² ≈ 15,1 м² |
| Лоджия (4,4 м²) | **2700 × 1700 мм** на юге, через раздвижное остекление из спальни | 2,7 × 1,7 = 4,59 м² (близко к 4,4; уточнять обмером парапета) |
| Толщина внутренних стен | **200 мм** | Как в design-book |
| Кровать | **1600 × 2000 мм** (queen), голова на **северной** стене (общая стена с en-suite — глухая, без двери) | Стандарт |
| Шкаф | **600 × 2500 мм** вдоль восточной стены | Глубина 600 — типовая для одежды |
| Высота потолка | **2700 мм** | §2.4 требований |

#### План (SVG · 1 ед. = 1 мм · север сверху)

<svg xmlns="http://www.w3.org/2000/svg" viewBox="-1700 -1100 6300 9700" role="img" aria-label="Parents wing plan: en-suite WC+basin+shower (1700×1850), L-shape bedroom 15.1 m² with queen bed, wardrobe along east wall, sliding glazing south to loggia 4.4 m². Dimensions in mm; verify on site.">
  <defs>
    <style>
      .pw-wall  { fill: #1a1a1a; }
      .pw-room  { fill: #fffdf8; stroke: #1a1a1a; stroke-width: 30; }
      .pw-wet   { fill: #e6edf4; stroke: #1a1a1a; stroke-width: 30; }
      .pw-ext   { fill: #f3efe4; stroke: #1a1a1a; stroke-width: 30; stroke-dasharray: 120 80; }
      .pw-rug   { fill: #ddd3bf; opacity: 0.55; }
      .pw-built { fill: #e2ecf4; stroke: #2c5282; stroke-width: 25; }
      .pw-soft  { fill: #f6f1e6; stroke: #2c5282; stroke-width: 25; }
      .pw-furn  { fill: none; stroke: #2c5282; stroke-width: 25; }
      .pw-door  { fill: none; stroke: #9c4221; stroke-width: 30; }
      .pw-leaf  { stroke: #9c4221; stroke-width: 30; }
      .pw-win   { stroke: #2c5282; stroke-width: 35; }
      .pw-lblrm { font: 700 230px system-ui, sans-serif; fill: #1a1a1a; text-anchor: middle; }
      .pw-lblsm { font: 600 150px system-ui, sans-serif; fill: #2c5282; text-anchor: middle; }
      .pw-lblar { font: 180px system-ui, sans-serif; fill: #5c5c5c; text-anchor: middle; }
      .pw-dimL  { stroke: #c53030; stroke-width: 18; }
      .pw-dimT  { stroke: #c53030; stroke-width: 18; marker-start: url(#tickL); marker-end: url(#tickL); }
      .pw-dlbl  { font: 600 145px system-ui, sans-serif; fill: #c53030; text-anchor: middle; }
      .pw-N     { font: 700 280px system-ui, sans-serif; fill: #2c5282; text-anchor: middle; }
      .pw-ttl   { font: 700 240px system-ui, sans-serif; fill: #1a1a1a; }
      .pw-note  { font: 160px system-ui, sans-serif; fill: #742a2a; }
      .pw-elec  { fill: #fff7d1; stroke: #b7791f; stroke-width: 25; }
      .pw-elblr { font: 600 130px system-ui, sans-serif; fill: #1a1a1a; }
    </style>
    <marker id="tickL" markerWidth="10" markerHeight="40" refX="5" refY="20" orient="auto">
      <line x1="5" y1="0" x2="5" y2="40" stroke="#c53030" stroke-width="18"/>
    </marker>
  </defs>

  <text class="pw-ttl" x="1450" y="-700" text-anchor="middle">Parents wing — derived plan (verify on site)</text>

  <!-- ========== EXTERNAL WALL MASS ========== -->
  <!-- Wing envelope: 2700 internal × 6750 internal + 200 walls each side -->
  <!-- Inside coordinates: 0..2700 (W-E), 0..6750 (N-S) -->
  <!-- Wall mass via fill-rule: outer 3100 × 7150 (offset -200,-200), inner cutout -->
  <path class="pw-wall" fill-rule="evenodd"
        d="M-200,-200 H2900 V6950 H-200 Z
           M0,0 H2700 V6750 H0 Z"/>

  <!-- ========== ROOM FILLS ========== -->
  <!-- Bedroom main fill -->
  <rect class="pw-room" x="0" y="0" width="2700" height="6750"/>

  <!-- En-suite at NW: 1700 × 1850 -->
  <rect class="pw-wet"  x="0" y="0" width="1700" height="1850"/>

  <!-- En-suite walls (south + east, with door cut on south) -->
  <rect class="pw-wall" x="0"    y="1650" width="700"  height="200"/>      <!-- S wall left of door -->
  <rect class="pw-wall" x="1500" y="1650" width="200"  height="200"/>      <!-- S wall stub right of door -->
  <rect class="pw-wall" x="1500" y="0"    width="200"  height="1650"/>     <!-- E wall of en-suite -->

  <!-- En-suite door (S wall, swings into bedroom) -->
  <path class="pw-door" d="M700,1850 A 800 800 0 0 1 1500 1050"/>
  <line class="pw-leaf" x1="700" y1="1850" x2="1500" y2="1850"/>

  <!-- ========== EN-SUITE FIXTURES ========== -->
  <!-- WC top-left -->
  <rect class="pw-furn" x="200" y="150" width="450" height="600" rx="50"/>
  <ellipse cx="425" cy="420" rx="180" ry="200" fill="none" stroke="#2c5282" stroke-width="25"/>
  <!-- Basin top-right -->
  <ellipse cx="1100" cy="350" rx="280" ry="170" class="pw-furn"/>
  <line class="pw-win" x1="800" y1="120" x2="1400" y2="120"/>
  <text class="pw-lblsm" x="1100" y="50">Mirror</text>
  <!-- Shower stall bottom of en-suite -->
  <rect class="pw-furn" x="200" y="900" width="800" height="700"/>
  <line class="pw-furn" x1="200" y1="900" x2="1000" y2="1600"/>
  <line class="pw-furn" x1="1000" y1="900" x2="200" y2="1600"/>
  <text class="pw-lblsm" x="600" y="1280">Shower</text>

  <!-- En-suite labels -->
  <text class="pw-lblrm" x="850" y="780" text-anchor="middle">En-suite</text>
  <text class="pw-lblar" x="850" y="980" text-anchor="middle">3.1 m² · 1700 × 1850</text>

  <!-- ========== BEDROOM FURNITURE ========== -->
  <!-- Rug under bed -->
  <rect class="pw-rug" x="350" y="2050" width="2000" height="2700"/>

  <!-- Queen bed 1600 × 2000, head against north wall (just south of en-suite line) -->
  <rect class="pw-soft" x="550" y="2150" width="1600" height="2000" rx="60"/>
  <!-- Pillows -->
  <rect class="pw-furn" x="600"  y="2200" width="700" height="200" rx="30"/>
  <rect class="pw-furn" x="1400" y="2200" width="700" height="200" rx="30"/>
  <!-- Bedside tables -->
  <rect class="pw-furn" x="100" y="2200" width="400" height="500"/>
  <rect class="pw-furn" x="2200" y="2200" width="400" height="500"/>

  <text class="pw-lblsm" x="1350" y="3300">Queen 1600 × 2000</text>

  <!-- Wardrobe along east wall, lower half -->
  <rect class="pw-built" x="2100" y="4500" width="600" height="2200"/>
  <text class="pw-lblsm" x="2400" y="5650" transform="rotate(90 2400 5650)">Wardrobe 600 × 2200 (full height)</text>

  <!-- Bedroom door from vestibule (north wall, east side, between en-suite east wall and bedroom east wall: x=1700..2500) -->
  <rect class="pw-room" x="1900" y="-200" width="800" height="200"/>
  <path class="pw-door" d="M2700,0 A 800 800 0 0 0 1900 800"/>
  <line class="pw-leaf" x1="2700" y1="0" x2="2700" y2="800"/>

  <!-- ========== SOUTH WALL: SLIDING DOOR TO LOGGIA ========== -->
  <!-- Cut south wall fully (south wall is at y=6750..6950) -->
  <rect class="pw-room" x="100" y="6750" width="2500" height="200"/>
  <line class="pw-win" x1="100"  y1="6850" x2="2600" y2="6850"/>
  <text class="pw-lblsm" x="1350" y="6720">Sliding glazing 2500 mm</text>

  <!-- ========== LOGGIA (external attached) ========== -->
  <rect class="pw-ext" x="0" y="6950" width="2700" height="1700"/>
  <text class="pw-lblrm" x="1350" y="7700" text-anchor="middle">Loggia</text>
  <text class="pw-lblar" x="1350" y="7900" text-anchor="middle">4.4 m² · 2700 × 1700</text>
  <!-- Built-in storage along east wall of loggia -->
  <rect class="pw-built" x="2100" y="7000" width="500" height="1600"/>
  <text class="pw-lblsm" x="2350" y="7800" transform="rotate(90 2350 7800)">Built-in storage</text>

  <!-- ========== BEDROOM LABEL ========== -->
  <text class="pw-lblrm" x="1100" y="5300">Bedroom · parents</text>
  <text class="pw-lblar" x="1100" y="5520">15.1 m² (L-shape: 1000×1850 + 2700×4900)</text>

  <!-- ========== ELECTRICAL POINTS (overlay) ========== -->
  <!-- Bedside outlets (×2) at ~750 mm AFF (plan view at floor) -->
  <circle class="pw-elec" cx="200"  cy="2400" r="100"/>
  <text class="pw-elblr" x="-200" y="2440">R · R · USB-C (750)</text>
  <circle class="pw-elec" cx="2500" cy="2400" r="100"/>
  <text class="pw-elblr" x="2620" y="2440">R · R · USB-C (750)</text>

  <!-- General outlet south wall left -->
  <circle class="pw-elec" cx="200"  cy="6300" r="100"/>
  <text class="pw-elblr" x="-280" y="6340">R · 300</text>

  <!-- LAN optional at desk wall (east wall north) -->
  <circle class="pw-elec" cx="2500" cy="3300" r="100"/>
  <text class="pw-elblr" x="2620" y="3340">LAN · опц.</text>

  <!-- AC indoor unit on north wall east of door (high) -->
  <rect class="pw-elec" x="1900" y="-80" width="600" height="160" rx="30"/>
  <text class="pw-elblr" x="1950" y="20">AC-B1 (2400)</text>

  <!-- Switch at door (east of door) -->
  <rect class="pw-elec" x="2600" y="900" width="180" height="180"/>
  <text class="pw-elblr" x="2820" y="1010">S-DIM · 900</text>
  <!-- Bedside dimmer dubl -->
  <rect class="pw-elec" x="100" y="2900" width="180" height="180"/>
  <text class="pw-elblr" x="-460" y="3010">S-DIM dubl L</text>
  <rect class="pw-elec" x="2420" y="2900" width="180" height="180"/>
  <text class="pw-elblr" x="2620" y="3010">S-DIM dubl R</text>

  <!-- En-suite switches OUTSIDE door (south of en-suite line, in bedroom) -->
  <rect class="pw-elec" x="1550" y="2000" width="180" height="180"/>
  <text class="pw-elblr" x="1100" y="2110">S-2 en-suite (snaruzhi)</text>

  <!-- En-suite light groups -->
  <circle class="pw-elec" cx="850" cy="500" r="100"/>
  <text class="pw-elblr" x="980" y="540">L-PEnSuite-Mirror</text>
  <circle class="pw-elec" cx="850" cy="1300" r="100"/>
  <text class="pw-elblr" x="980" y="1340">L-PEnSuite-Ceiling (IP65 в зоне душа)</text>

  <!-- Bedroom light groups -->
  <circle class="pw-elec" cx="900" cy="3700" r="100"/>
  <text class="pw-elblr" x="1030" y="3740">L-Bed1-Ambient</text>
  <circle class="pw-elec" cx="900" cy="4400" r="100"/>
  <text class="pw-elblr" x="1030" y="4440">L-Bed1-Read</text>

  <!-- Loggia light + IP44 outlet (TBD-E5 warm contour) -->
  <circle class="pw-elec" cx="900" cy="7700" r="100"/>
  <text class="pw-elblr" x="1030" y="7740">L-Loggia-P (TBD)</text>
  <circle class="pw-elec" cx="200" cy="8400" r="100"/>
  <text class="pw-elblr" x="-280" y="8440">R-WP IP44 (TBD-E5)</text>

  <!-- ========== DIMENSION LINES (red, outside the envelope) ========== -->
  <!-- Top: width 2700 -->
  <line class="pw-dimT" x1="0" y1="-450" x2="2700" y2="-450"/>
  <text class="pw-dlbl" x="1350" y="-510">2700 (W-E internal)</text>

  <!-- Left: en-suite height 1850 -->
  <line class="pw-dimT" x1="-450" y1="0" x2="-450" y2="1850"/>
  <text class="pw-dlbl" x="-450" y="950" transform="rotate(-90 -450 950)">1850</text>
  <!-- Left: bedroom main height 4900 (from y=1850 to y=6750) -->
  <line class="pw-dimT" x1="-450" y1="1850" x2="-450" y2="6750"/>
  <text class="pw-dlbl" x="-450" y="4300" transform="rotate(-90 -450 4300)">4900 (bedroom main)</text>
  <!-- Left: loggia height 1700 -->
  <line class="pw-dimT" x1="-450" y1="6950" x2="-450" y2="8650"/>
  <text class="pw-dlbl" x="-450" y="7800" transform="rotate(-90 -450 7800)">1700 (loggia)</text>

  <!-- Right: total internal length 6750 -->
  <line class="pw-dimT" x1="3150" y1="0" x2="3150" y2="6750"/>
  <text class="pw-dlbl" x="3150" y="3375" transform="rotate(90 3150 3375)">6750 (en-suite + bedroom)</text>

  <!-- En-suite width 1700 (top inside) -->
  <line class="pw-dimT" x1="0" y1="-100" x2="1700" y2="-100"/>
  <text class="pw-dlbl" x="850" y="-160">1700 (en-suite W-E)</text>

  <!-- Bedroom door 800 -->
  <line class="pw-dimT" x1="1900" y1="-100" x2="2700" y2="-100"/>
  <text class="pw-dlbl" x="2300" y="-160">800 door</text>

  <!-- ========== COMPASS ========== -->
  <g transform="translate(3700,-700)">
    <circle r="240" fill="none" stroke="#2c5282" stroke-width="35"/>
    <path d="M0,-220 L100,150 L0,70 L-100,150 Z" fill="#2c5282"/>
    <text class="pw-N" y="-300">N</text>
  </g>

  <!-- Scale bar 1000 mm -->
  <g transform="translate(3000,9100)">
    <rect width="1000" height="60" fill="#1a1a1a"/>
    <rect x="200" width="200" height="60" fill="#fff" stroke="#1a1a1a" stroke-width="20"/>
    <rect x="600" width="200" height="60" fill="#fff" stroke="#1a1a1a" stroke-width="20"/>
    <text class="pw-lblar" x="500" y="200" text-anchor="middle">1 m</text>
  </g>

  <!-- VERIFY note -->
  <text class="pw-note" x="-1500" y="9000">All dimensions derived from registry areas + owner reference image (2026-04-19) — verify on site (TBD-E10) before joinery and rough-in.</text>
</svg>

#### Что показано

- **En-suite (3,1 м²)** в северо-западном углу: WC слева, раковина с зеркалом справа, душевая кабина 800 × 700 мм внизу. Дверь — на южной стене, открывается в спальню.
- **Спальня (15,1 м², L-shape)**: вход с севера (восточный край, ширина 800 мм). Кровать **queen 1600 × 2000** головой на северной стене (общая с en-suite — глухая, без окна). Прикроватные тумбы по сторонам. Шкаф **600 × 2200** во всю высоту вдоль восточной стены, южная половина.
- **Лоджия (4,4 м²)** на юге, через раздвижное остекление 2500 мм; встроенное хранение 500 × 1600 вдоль восточной стены.
- **Электрика (жёлтые точки) — нанесена поверх**: см. ведомость в **§5.10.B** ниже.

<a id="section-5-10-b"></a>

#### 5.10.B Ведомость электрики парентского крыла

**1. En-suite (3,1 м²) — функция: парентский совмещённый узел WC + раковина + душ**

| Точка | Где | Высота | Кол-во | Примечание |
|-------|-----|--------|--------|------------|
| Свет потолок (вне зоны душа) | потолок en-suite | — | **1 группа L-PEnSuite-Ceiling** | **IP44+** в основной зоне; **IP65** в зоне 1 над душем — см. §5.6.A |
| Свет лица у зеркала | по бокам зеркала | 1700–1800 мм | **1 группа L-PEnSuite-Mirror** | **IP44+**, Ra ≥ 90 (требования §5.6) |
| Полотенцесушитель электрический (опц.) | у душа / у двери | питание за корпусом, **IP44+** | **0–1 DED** | **TBD-E6**; иначе — водяной по стояку |
| Розетка у раковины (бритва) | сбоку у зеркала | 1100 мм, **IP44+** | **0–1** | **УЗО** обязательно — у электрика |
| Выключатели — **снаружи** двери (в спальне, у косяка) | юг от en-suite | 900 мм | **1 (S-2 или S-3)** | Потолок + зеркало; полотенцесушитель — отд. |
| Вентиляция / вытяжка | потолок / шахта | — | — | Не дизайнерская точка; вентилятор — по электрику и УК |

**2. Спальня (15,1 м², L-shape) — функция: основная спальня родителей**

| Точка | Где | Высота | Кол-во | Примечание |
|-------|-----|--------|--------|------------|
| Прикроватные розетки L | стена W у тумбы L | 750 мм | **1 блок ×2** + USB-C | Зарядка, бра, увлажнитель |
| Прикроватные розетки R | стена E у тумбы R | 750 мм | **1 блок ×2** + USB-C | Идентично |
| Розетка общего назначения | южная стена у остекления | 300 мм | **1** | Пылесос, увлажнитель |
| Розетка у шкафа | восточная стена у шкафа | 300 мм | **1** | Гладильная доска / отпариватель |
| LAN — RJ45 (опц.) | восточная стена, центр | 300 / 1100 мм | **0–1** | По §5.0 / §5.1.1 — для второго WFH в спальне |
| Сплит — питание + дренаж | **северная** стена, восточная половина (не в створ кровати — кровать у северной стены, поэтому блок над дверным проёмом, поток в коридор) | 2400 мм | **1** AC-PWR + AC-DRN | **AC-B1**; ночной режим в спецификации |
| Выключатель у входной двери | у косяка, со стороны ручки | 900 мм | **1 (S-DIM)** | Основной свет, проходной с дублями у кровати |
| Дубль выключателя у кровати — L | у тумбы L | 900 мм | **1** | Радиодиммер допустим |
| Дубль выключателя у кровати — R | у тумбы R | 900 мм | **1** | Радиодиммер допустим |
| Свет фон | потолок над основной зоной | — | **1 группа L-Bed1-Ambient** | 2700–3000 K, диммер |
| Свет чтения | бра / прикроватные через прикроват. розетки | — | **1 группа L-Bed1-Read** *(опц. централ. упр.)* | 300 лк у изголовья |
| Шторы blackout (электропривод — опц.) | у южного остекления | питание за карнизом | **0–1** | **TBD-E7** заказчик |

**Логика выключателей в спальне (специфика L-shape):** входная дверь — в северо-восточной нише (за коридором мимо en-suite). Чтобы при входе сразу включить свет в основной зоне (южнее), `S-DIM у двери` управляет обеими группами: коротким нажатием — `L-Bed1-Ambient` 30 %, длинным — 100 %. **Дубль у кровати** — для отключения без вставания.

**3. Лоджия (4,4 м²) — функция: хранение + сезонная зона у спальни**

| Точка | Где | Высота | Кол-во | Примечание |
|-------|-----|--------|--------|------------|
| Розетка влагозащищённая | у внутренней (восточной) стены, низ | 300 мм, **IP44+** | **0–1** | **Только** при тёплом контуре и письме УК — **TBD-E5** |
| Свет потолок | — | — | **0–1 группа L-Loggia-P** | LED линейный или точечный; IP по факту контура |
| Выключатель | у двери (со стороны спальни) | 900 мм | **0–1** | По наличию света на лоджии |

#### Каскадные изменения по апартаменту (что ещё надо обновить)

Перенос **3,1 м²** из центрального холла в парентское крыло меняет апартамент-уровневую логику. Что **обязательно** скорректировать после подтверждения обмером:

1. **§5.7** этого файла — **СУ 1,6 м²** остаётся гостевым у входа (не меняется).
2. **§5.8** этого файла — **3,1 м²** теперь это **парентский en-suite** (см. **§5.10.A** выше); функция «душевая в холле» **исчезает** или переезжает (TBD-E10).
3. **§5.9** этого файла — **4,2 м²** теперь это **детский / основной санузел** (центральная позиция между холлом и спальней детей); пользователи — дети + гости.
4. **§4.1** оверлей плана — нужна перерисовка мокрого блока: en-suite в крыле родителей вместо «3,1 м² у холла». Полная перерисовка — **после обмера** (TBD-E10), пока — этот раздел **§5.10.A** считается **источником истины** для парентского крыла.
5. **`docs/apartment-93-design-book.html`** — план §4 (этажный) и parents-bedroom 1:75 (≈ строка 1102) — также **под обновление** после обмера; до этого момента отметить в design-book ссылку на **§5.10.A** настоящего файла как актуальную интерпретацию.
...
> **Заменён §5.10.A для парентского крыла.** Таблица ниже остаётся как **общий шаблон** для двуспальной спальни (например, для **§5.11** — спальни детей со взрослой кроватью или для гостевой конфигурации). Для **парентской** спальни — пользоваться **§5.10.A / §5.10.B**.
...
| **TBD-E10** | Парентское крыло — план по эталону заказчика (2026-04-19, см. **§5.10.A**) | Подтвердить обмером: ширина 2700 мм, длина крыла 6750 мм, en-suite 1700 × 1850 в NW, лоджия 2700 × 1700 на юге; уточнить **позицию двери в спальню** и **трассу дренажа AC-B1**. Каскадно: §4.1 оверлей и план design-book — под обновление (см. **§5.10.A** п. «каскадные изменения»). | До корпусной мебели и штробления спальни |

#### Артефакты под обновление после закрытия TBD-E10

После того как обмер на объекте подтвердит / скорректирует геометрию из **§5.10.A**, нужно синхронизировать следующие артефакты в репозитории — они **сейчас** показывают **прежнюю** интерпретацию (общую ванную, без en-suite у родителей):

| Файл / раздел | Что обновить | Приоритет |
|---|---|---|
| `img/apartment-93-electrical-layout.md` § **4.1** SVG (оверлей квартиры) | Перенести 3,1 м² из центрального холла в NW крыла родителей; пересобрать lighting-group теги, AC-B1 (новая позиция на северной стене спальни), LAN-точки крыла. | **Высокий** — до выпуска CD электрику |
| `docs/apartment-93-design-book.html` § 4 (план квартиры, ≈ строка с `viewBox="-1700 -1700 10500 18100"`) | Перерисовать мокрый блок и крыло родителей под новую интерпретацию; обновить ярлыки и площади. | **Высокий** — до печати owner-handover PDF |
| `docs/apartment-93-design-book.html` § 5.x (parents-bedroom plan 1:75, ≈ строка 1102) | Заменить на чистовой план парентской спальни, согласованный с **§5.10.A** настоящего файла. | **Высокий** |
| `docs/apartment-93-design-book.html` § 5.x (бат-схемы, IP-зоны) | Добавить отдельный лист en-suite (3,1 м²) с IP-зонами — сейчас 3,1 м² трактуется как «душевая в холле». | **Средний** |
| `img/apartment-project-requirements.md` § 5.8.5–5.8.7 (функциональное распределение мокрых) | По решению заказчика — переписать функцию: 1,6 = гостевой WC, 3,1 = парентский en-suite, 4,2 = детский / основной санузел. | **Средний** — после устной фиксации |
| Деливерабл-матрица `docs/apartment-93-deliverables-matrix.html` | Отметить статус **§5.10.A** и связанные обновления. | **Низкий** — формальная пометка |

> До тех пор, пока эти артефакты не обновлены, **источник истины для парентского крыла** — **§5.10.A** настоящего файла.
